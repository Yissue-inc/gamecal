...
function setLang(l){
  if(l!=='ko' && l!=='en') return;
  LANG = l;
  /* ⚠ 조각 치환 결과는 캐시된다 — 언어가 바뀌면 그 캐시는 옛 언어의 답이다.
     지금은 ko 에서 K() 가 먼저 반환해 문제가 안 되지만, 캐시를 남겨 둘 이유가 없다. */
  try{ if(typeof _subCache!=='undefined') _subCache.clear(); }catch(e){}
  try{ localStorage.setItem(I18N_KEY, l); }catch(e){}
}

/* ⛔ **저장할 때 K() 를 부르면 그 순간의 언어로 굳는다.**
   실측(2026-08-31): 영어로 하다 한국어로 바꾸면 'The hall is empty' 가 그대로 남았다
   (반대 방향은 멀쩡하다 — 한국어 원문은 그릴 때 번역되니까).
   화면에 나갈 때 txt() 가 K() 를 부른다 — **원문(한국어)을 저장한다.**
   ⚠ 이름·낱말을 끼워 넣는 문구(`K('… %1 …').replace('%1', 이름)`)는 예외다:
      자리표를 채운 뒤에는 표를 못 찾으므로 그때는 저장 시점에 번역해야 한다.
      (숫자만 끼우는 것은 _tmpl 이 되돌리므로 원문 저장이 가능하다.) */
