/* ══════════════════════════════════════════════════════════════════
   AI 난이도 — 라이벌이 얼마나 센가 (CK 제안 2026-08-30)

   ⚠ 왜 필요한가
     라이벌 실력이 종목마다 **하드코딩**돼 있었다(`0.62 + i*0.16 + rand*0.1`).
     처음 켠 사람에게는 3위가 기본값이고, 익숙해진 사람에게는 늘 이길 상대다.
     둘 다 재미가 없다 — 한쪽은 벽이고 한쪽은 심심하다.

   ⛔ 이건 **기록에 손대지 않는다.** 내 기록·메달·기준은 난이도와 무관하다 —
      난이도를 낮춰 금메달을 딸 수 있으면 금이 금이 아니다.
      난이도가 바꾸는 것은 **누구와 나란히 뛰는가** 하나뿐이다.
      (그래서 '쉬움'으로 놓아도 기록이 부풀지 않는다 — 상대만 느려진다)

   ⚠ 값은 실측 위에 얹었다. 지금 기본값(보통)이 0.62~0.94 구간을 쓴다.
      쉬움 −0.14 / 어려움 +0.12 — 어려움은 사람의 완벽한 박자(±0ms)와 겨룰 만하게.
   ══════════════════════════════════════════════════════════════════ */
'use strict';

const AI = {
  KEY: 'wsc_ai',
  /* off 는 없다 — 혼자 뛰면 순위가 없어져 '3위'라는 결과가 무의미해진다 */
  LEVELS: ['easy', 'normal', 'hard'],
  NAME:   { easy:'쉬움', normal:'보통', hard:'어려움' },
  /* 라이벌 실력에 더하는 값 */
  BONUS:  { easy:-0.14, normal:0, hard:+0.12 },
  /* 라이벌의 손떨림 배수 — 어려울수록 정확해진다 */
  JITTER: { easy:1.35, normal:1.0, hard:1.000 },

  level: 'normal',

  load(){
    try{ const v=localStorage.getItem(this.KEY);
         if(this.LEVELS.indexOf(v)>=0) this.level = v; }catch(e){}
    return this.level;
  },
  set(l){
    if(this.LEVELS.indexOf(l)<0) return;
    this.level = l;
    try{ localStorage.setItem(this.KEY, l); }catch(e){}
  },
  cycle(){ this.set(this.LEVELS[(this.LEVELS.indexOf(this.level)+1) % this.LEVELS.length]); },
  get label(){ return this.NAME[this.level] || this.NAME.normal; },

  /* 종목이 계산한 기본 실력에 난이도를 얹는다.
     ⚠ 0.30~1.00 으로 묶는다 — 아래로 새면 라이벌이 걸어가고, 위로 새면 사람이 못 이긴다. */
  skill(base){
    return clamp(base + (this.BONUS[this.level] || 0), 0.30, 1.00);
  },
  jitter(base){
    return base * (this.JITTER[this.level] || 1);
  },

  /* ⛔ **사람 기록 대비 배수** — 시간 공간의 난이도(0E_paceskill 과 짝).
     단거리는 skill 에 상수를 더하면 1위 확률이 0%↔100% 로 튄다(곡선이 가파르고 포화한다).
     사람의 성능 띠가 9.57s(완벽)~10.49s(±40ms) 라, 라이벌을 그 **띠 안** 에 놓는다.
       쉬움  2.000 → 19.14s · **아이용** — 박자가 거칠어도 1위(자기 기록 깨기가 주인공)
       보통  1.045 → 10.00s · ±20ms 는 이기고 ±40ms 는 진다
       어려움 1.000 →  9.57s · 라이벌이 사람의 완벽한 기록과 **동률** — 반반 싸움
       ⛔ 1.00 아래로 내리지 말 것 — 완벽하게 뛰어도 지는 '불가능' 이 된다(실측으로 확인된 선)
     ⛔ 어려움을 1.00 미만으로 놓으면 '어려움'이 아니라 '불가능'이다. 실측으로 확인된 선이다. */
  /* ⛔ 쉬움은 **아이가 하는 모드**다(CK 2026-08-30). 1.090 은 어른이 박자를 잘 맞춰야
     겨우 이기는 값이었다 — 실측으로 ±40ms 면 반반, ±90ms(아이 수준)면 진다.
     아이는 라이벌을 이기는 게 아니라 **자기 기록을 깨는** 재미로 논다.
     ⚠ 값은 아이 박자를 **실측해서** 잡았다(짐작하지 않았다):
        ±90ms → 14.8s · ±150ms → 16.5s(최악 17.4s) · ±220ms 는 완주 실패도 나온다.
        1.55(14.8s)로는 아이가 여전히 3위였다 → **2.00 = 19.1s**. 거칠어도 1위로 들어온다. */
  PAR_TARGET: { easy:2.000, normal:1.045, hard:1.000 },
  parRatio(){ return this.PAR_TARGET[this.level] || this.PAR_TARGET.normal; },

  /* ── 연타 모드의 라이벌 박자 (2026-08-31) ───────────────────────────
     ⛔⛔ **연타 모드로 바꾸면서 라이벌은 옛 박자 모델에 남겨 뒀다.**
        사람은 `RULES.mashMode` 라 **빨리 칠수록 빠른데**, 라이벌은
        `r.targetIntervalMs()` 로 친다 — 그건 박자 모드의 '목표 간격'이다.
        실측(브라우저·실제 이벤트):
          · 라이벌 100m 은 실력을 1.00 까지 올려도 **12.33s 가 한계**
            (실력 0.30 은 13.44s — 전 구간이 1.1초 안에 뭉쳐 있다)
          · 사람은 9.8s(14타/초)~14.3s(3.8타/초) 를 오간다
          → 보통과 어려움이 사실상 같은 게임이었다(실측 19.0m vs 17.7m 차이)
          · 200m 이상에서는 목표 간격이 부풀어 **라이벌이 완주조차 못 한다**
            (사람이 먼저 들어와 경기가 끝나기 때문에 화면에서는 안 보였다)
        ⚠ 통제 실험: 같은 스탯·같은 연타 간격이면 사람과 라이벌 기록이 **완전히 같다**
          (100m 10.52 vs 10.52 · 200m 21.28 vs 21.28 · 400m 44.17 vs 44.17).
          물리는 대칭이다 — 라이벌에게 부족했던 건 **박자 하나**였다.
     그래서 난이도가 라이벌의 **연타 간격**을 정한다. 손잡이는 하나뿐이다.
       쉬움  300ms(3.3타/초) — 아이(3.8타/초·14.3s)가 이긴다
       보통  150ms(6.7타/초) — 초당 9타쯤 쳐야 이긴다
       어려움 110ms(9.1타/초) — 초당 11타는 쳐야 이긴다
     ⚠ 거리별 표가 필요 없다 — 라이벌도 사람과 같은 피로 곡선을 타므로
        같은 간격이 거리마다 알아서 다른 기록이 된다. */
  MASH_IV: { easy:300, normal:150, hard:110 },
  mashIv(){ return this.MASH_IV[this.level] || this.MASH_IV.normal; },

  /* ⛔ **맞붙는 종목(펜싱·유도·탁구)에는 난이도를 걸지 않는다.**
     거기선 상대가 곧 종목이라, 상대를 바꾸면 내 기록이 함께 바뀐다 —
     쉬움으로 금을 사고(탁구 쉬움은 전부 금), 어려움에선 **이기고도 기준 미달**이었다(204~224s vs 140s).
     난이도가 바꾸는 것은 '누구와 **나란히** 뛰는가' 하나뿐이다.
     ⚠ 그래서 48종목 중 3종목은 난이도가 없다 — 없는 게 맞아서 없다. */
};
