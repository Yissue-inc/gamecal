...
const FOCUS = {
  ...
  /* ⚠ 설명을 화면 쪽 삼항식에 두면 항목을 더할 때마다 어긋난다 —
     면담을 넣자마자 '부상 회복이 2배 빨라진다'로 떴다. 설명은 여기 산다. */
  rest        : { name:'휴식',    stat:null,           load:-2.2, desc:'피로를 크게 회복한다' },
  care        : { name:'치료·관리', stat:null,         load:-1.2, desc:'부상 회복이 2배 빨라진다' },
  /* ── 면담 ────────────────────────────────────────────────
     ⚠ 사기는 한 시즌에 **23~100 으로 흔들리고 성장을 27.6% 좌우한다**(실측).
        시설을 만렙(+17.5%)까지 올리는 것보다 크다. 그런데 **플레이어 손잡이가
        하나도 없었다** — 알아서 오르내렸다. 숨은 채로 제일 크게 흔드는 값이었다.
     ⛔ 새 자원도 새 화면도 안 만든다. 훈련 지시 3칸 중 하나를 쓰는 선택지로 넣는다 —
        면담을 하면 그 주에 한 명을 덜 훈련시킨다. 그게 값이다. */
  /* ⚠ 처음엔 **개인 면담**으로 만들었다가 실측하고 버렸다:
        사기는 크게 올랐는데(최저 23→44) OVR 은 오히려 **−0.20** 이었다.
        지도 슬롯 하나를 통째로 쓰는 값을 한 명분 사기로는 못 갚는다 —
        아무도 안 고를 선택지는 없느니만 못하다.
     → **팀 미팅**으로 넓힌다. 한 칸을 쓰되 효과는 선수단 전체에 간다.
        감독이 하는 일로도 더 자연스럽다. */
  talk        : { name:'팀 미팅',  stat:null,           load:-0.4, talk:true,
                  desc:'선수단 전체의 사기를 올린다 — 낮은 선수일수록 크게' },
};
