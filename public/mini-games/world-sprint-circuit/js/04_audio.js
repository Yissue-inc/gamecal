...
const Sfx = {
  ...
  /* ⛔ 메달 3단계를 넣었는데 **소리는 그대로**였다 — 금과 동이 같은 소리로 끝났다.
     성취의 크기가 소리에 안 실리면 그 성취는 절반만 온다.
     ⚠ 같은 세계의 소리여야 한다 — 음정만 올리고 길이만 늘린다(다른 악기를 쓰지 않는다).
        동 = 지금까지의 finish 그대로. 은 = 한 음 더. 금 = 팡파르 + 관중. */
  medal(kind){
    if(kind==='gold'){
      [880,1108,1318,1760].forEach((f,i)=>setTimeout(()=>this.beep(f,0.30,'square',0.16),i*95));
      setTimeout(()=>{ this.beep(2093,0.42,'square',0.13); this.roar(); }, 400);
    } else if(kind==='silver'){
      [880,1108,1318,1568].forEach((f,i)=>setTimeout(()=>this.beep(f,0.24,'square',0.14),i*88));
    } else {
      this.finish();
    }
  },

  /* ⛔ 여기 '이 게임의 핵심은 얼마나 빨리가 아니라 얼마나 고르게다(목표 238ms)' 라고
     적혀 있었다. **연타 모델(RULES.mashMode)로 바꾼 뒤로는 반대다** —
     실측(2026-09-04): 같은 평균 간격에서 흔들림 ±0 vs ±45ms 는 10.27 vs 10.36(1%),
     간격 90ms vs 240ms 는 10.27 vs 13.71(33%). **타수가 속도이고 고름은 거의 무관하다.**
     ⚠ 그래서 이 딸깍은 '맞춰야 할 박자'가 아니라 **최소한 이만큼은 쳐라**는 하한선이다.
        콤보가 붙으면 스스로 물러난다(아래) — 잘 치는 사람에겐 잔소리라서.
