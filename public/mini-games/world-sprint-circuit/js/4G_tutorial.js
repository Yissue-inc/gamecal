...
const Tutorial = {
  ...
  STEPS: [
    ...
    /* ⛔ '개인 최고를 깨면 커리어가 오릅니다' 였다 — 부정확하다.
       `Career.finishRace` 는 **뛸 때마다** 올린다(`add(ok?12:4)`), 개인 최고는 **추가 +25** 다.
       옛 문장은 '최고를 못 깨면 헛수고' 로 읽힌다 — 처음 오는 사람을 쫓아낸다. */
    { say:'기록이 남습니다',
      hint:'뛸 때마다 커리어가 오릅니다 — 개인 최고를 깨면 더 크게',
      hold:2600 },
    ...
    /* ⛔ 여기 '주를 넘기면 대회 주가 옵니다' 라고 적혀 있었다(2026-09-07).
       튜토리얼은 `MG.newGame` 으로 **1주차**에서 시작하고 대회는 `MEET_WEEKS = {6,12,18,24}` 다 —
       시킨 대로 한 주를 넘기면 **아무 일도 안 일어난다**(5주가 남았다).
       처음 오는 사람이 '내가 뭘 잘못했나' 로 읽는다. 화면은 이미 답을 갖고 있다 —
       사무실의 「다음 주로」 줄이 `N주차 대회까지 N주` 를 말한다(41_mgr_ui:538). 그걸 가리킨다. */
    { say:'대회에서 승점을 법니다',
      hint:'「다음 주로」가 대회까지 몇 주인지 알려 줍니다 — 대회 주에 출전표를 짭니다',
      done(){ return true } },
  ],
