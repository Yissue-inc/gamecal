...
const RING = {
  ...
  /* 되잡을 때 반대로 밀리는 양 — **필요 없을 때 누르면 그만큼 넘어간다.**
     ⚠ 값을 고르기 전에 링을 제대로 칠 수 있는 드라이버부터 만들어야 했다
        (그 전 드라이버는 **액션을 안 눌러 자세를 못 넘겼다** — 첫 자세에서 13.77초를
         버티다 42초 제한에 걸렸고, 그래서 링 점수가 늘 3.88~5.12 로 나왔다. 기준은 11.7).
     실측 2026-09-04 · 되잡기 임계별 중앙값(6판) — 대가 없음 → 0.13:
        임계 0.10  12.83 → 12.46   (탭 64)
        임계 0.18  13.84 → 12.59   (탭 38)
        임계 0.25  14.26 → 13.62   (탭 19)   ← 대가 전의 최적
        임계 0.35  14.23 → 14.16   (탭  7)   ← 대가 후의 최적
        연타       12.45 → 12.19   (탭 159)
     즉 **최적이 19탭에서 7탭으로 옮겨갔다** — 필요한 만큼만 누르는 쪽이 이긴다.
     천장은 14.26 → 14.16 으로 거의 그대로라 금(13.36)·기준(11.7)은 그대로 닿는다.
     무너진 자세는 어느 설정에서도 0 이었다 — 벌이 가혹한 게 아니라 **방향을 준다.** */
  overshoot: 0.13,
...
class RingsEvent {
  ...
  onStride(side){
    ...
    if(helping){
      ...
      /* ⛔ 되잡으면 반대로 조금 넘어간다 — **필요한 만큼만 누르라**는 대가다.
         원래 이 자리는 `this.vel += dir * RING.overshoot` 이었는데 `vel` 을 아무도 안 읽어
         **아무 일도 안 하고 있었다**(흔들림 물리를 다시 짤 때 이 줄만 남았다).
         2026-09-04 죽은 값 검사로 적발 → CK 지시로 되살렸다. 이번엔 `wob` 에 직접 건다. */
      this.wob += dir * RING.overshoot;
      /* (아래는 되살리기 전에 적어 둔 경위 — 지우지 않고 남긴다)
         ...
         ⚠ 지워서 되살리지 않는다 — 값을 새로 정하려면 이 종목을 사람이 칠 수 있는
            ...
            못 재는 값을 지어내지 않으려고 그때는 없는 채로 두었다. */
      Sfx.step('GOOD');
    } else {
      this.wob += dir * RING.nudge * 0.6;      // 반대쪽을 누르면 더 밀린다
      this.wob += dir * RING.overshoot * 1.4;  // 게다가 넘어가는 양도 크다
      Sfx.step('MISS');
    }
    this.taps++;
  }
  ...
  drawUI(u){
    ...
    txtOn(u, K('흔들림'), bx0-8, by-1, 9, PAL.dim, 'right');
    ...
    txtOn(u, K('되잡기 %1').replace('%1', this.taps), 8, Track.botY(19), 9, over>0?PAL.red:PAL.dim);
    if(over>0) txtOn(u, K('너무 자주 잡으면 몸이 출렁인다'), 8, Track.botY(9), 8, PAL.red);
    ...
    txtOn(u, K('좌·우로 되잡아 흔들림을 0 에 둔다'), 8, Track.botY(32), 9, PAL.dim, 'left');
