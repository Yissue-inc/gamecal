...
class MiddleEvent {
  ...
  reset(){
    ...
    /* ⛔⛔ **라이벌 기준은 `rivalPar` 다 — `parS` 가 아니다.**
       parS 는 세 가지를 겸하고 있다: ①은메달 컷(medalCuts) ②감독 모드 기록 앵커
       ③여기(라이벌 속도). 2026-08-31 에 중장거리를 **연타 모델로 갈아엎으면서
       사람이 낼 수 있는 기록이 통째로 바뀌었는데 parS 는 옛 박자 모델 값 그대로였다.**
       실측(드라이버 최적화 탐색 — 페이스·케이던스·스퍼트 전부 훑음):
         800m   사람최선 139.3s  vs parS 127.0   → 보통 라이벌 132.7s **이길 수 없음**
         1500m         270.0s  vs      238.0   → 보통 248.7s      이길 수 없음
         5000m        1011.1s  vs      792.0   → 보통 827.6s      이길 수 없음
         20km경보    23,701s  vs     7,800    → 라이벌이 **3배 빠름**
         마라톤      16,491s  vs     6,720    → 라이벌이 2.5배 빠름
       화면에서는 안 보였다 — 라이벌은 그냥 앞서 갈 뿐이고, 사람은 '내가 못해서'라고 읽는다.
       ⚠ parS 는 건드리지 않는다(메달 컷·감독 앵커가 매달려 있다). 계주·클라이밍이 이미
         쓰던 `rivalPar` 관용구를 그대로 쓴다 — 라이벌 기준만 따로 잰 값이다.
       ⚠ 고친 뒤 실측한 사다리(잘함=최적 드라이버 · 보통=1.45배 느린 손 · 초보=2.4배):
         800·1500·5000 → 잘함 전부 1위 · 보통 easy·normal 1위 · 초보 easy 만 1위
         경보·마라톤   → 잘함 전부 1위 · 보통·초보 easy 만 1위 (실력 폭이 훨씬 넓다)
       ⚠ 남은 아쉬움: **어려움이 '반반'이 아니라 잘하는 손에겐 100% 다.**
         기준을 사람 최선에 맞췄기 때문이다. 더 조이면 `PAR_TARGET.hard < 1.00` 이 되는데
         그건 '어려움'이 아니라 '불가능'이라 0C_difficulty 가 ⛔ 로 막아 둔 선이다.
         사람은 드라이버처럼 매번 최선을 못 낸다 — 그 분산이 반반을 만든다. */
    ...
    for(let i=humans;i<lanes;i++){
      /* ⛔ 예전엔 AI.pace(실력폭 비례) 를 썼다 — 시간으로 ±17.5% 를 움직인다.
         ...
         ⚠ 기준값은 rivalPar 다(위 주석) — 모델을 바꾸면 **여기도 다시 재야 한다.** */
