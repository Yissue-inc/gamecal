/* 영어 번역표 — 키는 한국어 원문. 숫자는 %1,%2… 자리표로 정규화된다.
   ...
   ⚠ 사람 이름은 번역하지 않는다. 이름은 **국적**을 따라간다(30_athlete.js:198) —
      한국 클럽에는 한국 이름 선수가 있고, 영어판에서도 그대로다. FM 과 같은 규칙이다.
      (이 줄은 한때 '영어판은 NAME_EN 만 쓴다' 라고 **거짓 주장**을 하고 있었다)
   ⚠ 종족 이름은 반대다 — `Athlete.speciesName` 이 **미리 번역해서** 돌려준다.
      화면들이 `${speciesName} ${name}` 로 조립하는데, 조립된 뒤에는 번역이 안 된다.
...
const I18N_EN = {
  ...
  /* ⛔ 2026-08-31 — i18n 검사기가 **데이터 표 안의 화면 문구**를 못 봐서 108건이 새고 있었다.
     (부상명·아이템·코치·스킬 설명·사건 선택지 — 감독 모드가 통째로 한국어였다) */
  '음악':'Music',
  '햄스트링 염좌':'Hamstring strain',
  '발목 염좌':'Ankle sprain',
  '종아리 근육통':'Calf strain',
  '무릎 통증':'Knee pain',
  '허리 통증':'Back pain',
  '아킬레스건염':'Achilles tendinitis',
  '▶ 경기 시작':'▶ Start the meet',
  '출전표를 확정하고 경기를 본다':'Lock the entries and watch',
  '자동 편성':'Auto-fill',
  '컨디션과 적합도로 자동으로 짠다':'Fills by condition and fit',
  '영입한다':'Sign',
  '보류':'Hold',
  '(선수 없음)':'(no athlete)',
  '수락한다 — 보낸다':'Accept — let them go',
  '거절한다':'Decline',
  '일반':'Common',
  '고급':'Fine',
  '희귀':'Rare',
  '스파이크':'Spikes',
  '쿠션 밑창':'Cushioned soles',
  '경기복':'Race suit',
  '압박 타이츠':'Compression tights',
  '페이스 시계':'Pace watch',
  '테이핑':'Taping',
  '저항 밴드':'Resistance band',
  '단거리 코치':'Sprint coach',
  '웨이트 코치':'Strength coach',
  '지구력 코치':'Endurance coach',
  '기술 코치':'Technique coach',
  '리듬 코치':'Rhythm coach',
  '의무 트레이너':'Physio',
  '출발 반응이 빨라진다':'Quicker off the gun',
  '출발이 눈에 띄게 빠르다':'Explosive start',
  '후반에 덜 무너진다':'Fades less late',
  '마지막까지 페이스가 산다':'Holds pace to the line',
  '리듬이 덜 흔들린다':'Steadier rhythm',
  '리듬이 거의 안 흔들린다':'Rock-steady rhythm',
  '큰 경기일수록 잘한다':'Rises for big meets',
  '허들을 깔끔하게 넘는다':'Clears hurdles cleanly',
  '도약이 멀리 뻗는다':'Jumps carry further',
  '던지기가 강해진다':'Throws land longer',
  '피로가 잘 빠진다':'Recovers faster',
  '부상 위험이 준다':'Lower injury risk',
  ...
  '명 지구력':' athletes stamina',
  '이번엔 넘어갔다':'You passed this time',
  '전원 컨디션':'Squad condition',
  '출전시켜 주세요':'Put me in',
  '약속한다':'Promise them a spot',
  '사기 크게 오름 · 화합 −(다른 선수가 본다)':'Big morale gain · spirit −(others notice)',
  '실력으로 따내라':'Earn it',
  '사기 − · 화합 +(원칙이 선다)':'Morale − · spirit +(standards hold)',
  '따로 훈련을 붙여 준다':'Give them extra coaching',
  '자금 −18 · 사기와 기량 조금':'Funds −18 · a little morale and skill',
  '기자가 묻는다':'A reporter asks',
  '반드시 해낸다':'We will do it',
  '명성 + · 선수들이 부담을 진다(사기 −)':'Reputation + · the squad carries it (morale −)',
  '선수들을 믿는다':'I trust my athletes',
  '화합 + · 명성 변화 없음':'Spirit + · reputation unchanged',
  '대답을 피한다':'Dodge the question',
  '명성 − · 아무 일도 없음':'Reputation − · nothing else',
  '후원 제안':'Sponsorship offer',
  '받는다':'Take it',
  '명성 +0.2 · 돈은 없다':'Reputation +0.2 · no money',
  '라이벌의 말':'A rival speaks',
  '받아친다':'Answer back',
  '화합 + · 명성 + · 목표 부담(사기 −)':'Spirit + · reputation + · pressure (morale −)',
  '무시한다':'Ignore it',
  '아무 일도 없다':'Nothing happens',
  '훈련장에 붙여 놓는다':'Pin it up in the gym',
  '전원 사기 + · 화합 −(누군가는 싫어한다)':'Squad morale + · spirit −(someone hates it)',
  '라커룸이 시끄럽다':'Trouble in the locker room',
  '둘을 앉혀 놓고 푼다':'Sit them down',
  '화합 +10 · 이번 주 훈련 손해(둘 컨디션 −)':'Spirit +10 · training lost this week (both condition −)',
  '내버려 둔다':'Leave it alone',
  '화합 −12 · 아무 손해 없음':'Spirit −12 · no other cost',
  '한 명을 편든다':'Take a side',
  '한쪽 사기 ++ · 다른 쪽 −− · 화합 −':'One morale ++ · the other −− · spirit −',
  '괜찮다고 합니다':'They say they are fine',
  '쉬게 한다':'Rest them',
  '컨디션 크게 회복 · 사기 −':'Big condition gain · morale −',
  '본인 말을 믿는다':'Take their word',
  '사기 + · 부상 위험을 안는다':'Morale + · you carry the injury risk',
  '정밀 검진을 받게 한다':'Send them for a scan',
  '자금 −25 · 확실히 회복':'Funds −25 · a certain recovery',
  '합숙 제안':'Training camp offer',
  '간다':'Go',
  '보낸다':'Pass',
  '관중이 줄었다':'The stands are emptying',
  '홈 트랙에 사람이 없습니다. 무언가 해야 합니다.':'Nobody is coming to the home track. Something must change.',
  '공개 훈련을 연다':'Open the training session',
  '명성 +0.5 · 이번 주 훈련 손해':'Reputation +0.5 · training lost this week',
  '성적으로 말한다':'Let results speak',
  '화합 +5':'Spirit +5',
  '앞에서 끌고 간다 — 안정적이지만 상한이 낮다':'Lead from the front — steady, lower ceiling',
  '고르게 간다 — 기본':'Hold an even pace — default',
  '아끼다 막판에 쏟는다 — 잘 되면 최고, 안 되면 최악':'Save it and kick — best or worst',
  /* 클럽 사건 본문 — evText 패턴(%A 등은 코드가 채운다) */
  "%A 이(가) 다음 대회에 꼭 나가고 싶다고 합니다.":"%A really wants a place at the next meet.",
  "\"올해 목표가 승점 %P·금 %G 라던데, 자신 있습니까?\"":"\"They say the target is %P points and %G gold. Confident?\"",
  "지역 기업이 %N 을 제안합니다. 대신 클럽 이름 옆에 로고가 붙습니다.":"A local firm offers %N. Their logo goes beside the club name.",
  "%C 감독: \"그 클럽은 올해도 우리 상대가 아니죠.\"":"%C manager: \"That club is no match for us again this year.\"",
  "%A 와 %B 가 부딪쳤습니다.":"%A and %B have clashed.",
  "%A 의 몸 상태가 안 좋아 보이는데 본인은 뛰겠다고 합니다.":"%A looks in poor shape but insists on racing.",
  "고지대 합숙 자리가 났습니다. 비용 %N.":"An altitude camp place has opened up. Cost %N.",
  /* 계주 주자 순서(42_meet_ui) */
  '번 주자':' leg',
  '마지막':'anchor',
  '확인 선택/해제   ◀▶ 주자 순서   취소 돌아가기':'Confirm toggle   ◀▶ Leg order   Cancel back',
  /* 레이스 플랜(4J_tactics) */
  '플랜':'Plan',
  '선행':'Front-run',
  '균등':'Even',
  '추입':'Kick',
  '섞임':'Mixed',
  '확인 선택/해제   ◀▶ 레이스 플랜   취소 돌아가기':'Confirm toggle   ◀▶ Race plan   Cancel back',
  '초반부터 앞선다 — 지구력이 받쳐야 한다':'Lead from the gun — needs stamina',
  '고르게 간다':'Hold an even pace',
  '아끼다 막판에 쏟는다 — 앞이 멀면 못 잡는다':'Save it for the finish — too far back and you miss',
  /* 사무실 '지금 할 것' */
  '일일 도전 보상을 받으세요':'Claim your daily challenge reward',
  '세 종목을 다 마쳤습니다':'You finished all three events',
  '도감 보상을 받으세요':'Claim your codex reward',
  '이정표에 닿았습니다':'You reached a milestone',
  '출전표를 짜세요':'Set your entries',
  '치료 지도는 회복이 2배 빠릅니다':'Rehab coaching heals twice as fast',
  '남은 자리를 쓰지 않으면 그냥 사라집니다':'Unused slots are simply lost',
  '배웠는데 장착을 안 했습니다':'Learned but not equipped',
  '시설을 지을 수 있습니다':'You can build a facility',
  /* 경기 전 팀 지시(4H_clublife) */
  '경기 전 지시':'Team talk',
  '출전':'Entered',
  '출전 선수 평균 사기':'Average morale of entrants',
  '맞는 선수':'Fits',
  '사기 오름':'Lifted',
  '사기 내림':'Knocked',
  '확인 경기 시작':'Confirm to start',
  '몰아붙인다':'Demand more',
  '편하게 하라':'Take the pressure off',
  '사기가 높을 때만 먹힌다':'Only works when morale is high',
  '언제나 조금은 오른다':'Always lifts a little',
  '사기가 낮을 때 살린다':'Revives a low-morale squad',
  '확인 지시   ▲▼ 고르기':'Confirm to talk   ▲▼ Choose',
  '경기 시작':'Start the meet',
  /* 갈래 전용 훈련(4M_identity) */
  '지구력에 막판 스피드를 붙인다':'Endurance with a finishing gear',
  '갈래 전용':'Identity only',
  '스타트 블록':'Blocks',
  '출발 반응과 초반 가속':'Reaction off the gun and early drive',
  '플라잉 30m':'Flying 30m',
  '최고 속도 구간만 되풀이':'Top-speed reps only',
  '허들 간격':'Hurdle spacing',
  '세 걸음 리듬을 몸에 넣는다':'Drill the three-step rhythm',
  '반응 훈련':'Reaction drills',
  '신호에 즉시 — 펜싱·탁구에도 듣는다':'Instant on the cue — helps fencing and table tennis',
  '장거리 지구주':'Long steady runs',
  '천천히 오래 — 피로가 적다':'Slow and long — low fatigue',
  '인터벌':'Intervals',
  '빠르게·쉬고 되풀이. 힘들지만 크게 는다':'Hard, rest, repeat — brutal but it builds',
  '플라이오메트릭':'Plyometrics',
  '튀어 오르는 힘':'Bounce and spring',
  '공중 자세':'Air position',
  '몸을 다루는 법 — 체조·다이빙에도 듣는다':'Body control — helps gymnastics and diving',
  '웨이트':'Weights',
  '순수한 힘. 피로가 크다':'Raw strength — heavy fatigue',
  '정밀 훈련':'Precision work',
  '같은 동작을 흔들림 없이 — 사격·양궁':'The same motion without a tremor — shooting and archery',
  '스트로크':'Stroke work',
  '물을 잡는 법':'How to hold the water',
  '세트 훈련':'Set training',
  '거리를 나눠 되풀이 — 버티는 힘':'Split the distance and repeat — staying power',
  ...
  '나머지':'the rest',
  '우리 선수':'ours',
  '시즌 중에는 바꿀 수 없습니다':'Cannot change mid-season',
  '단거리부':'Sprint squad',
  '100m·200m·계주 — 폭발력':'100m · 200m · relays — explosiveness',
  '허들·기술부':'Hurdles & touch squad',
  '허들·펜싱·탁구·승마 — 반응과 리듬':'Hurdles · fencing · table tennis · showjumping — reaction and rhythm',
  '중장거리부':'Distance squad',
  '중장거리·경보·사이클·조정 — 버티기':'Distance · walk · cycling · rowing — endurance',
  '도약부':'Jump squad',
  '도약·체조·다이빙·등반 — 몸을 다루는 힘':'Jumps · gymnastics · diving · climbing — body control',
  '투척·정밀부':'Throw & precision squad',
  '투척·역도·사격·양궁·유도 — 힘과 정밀':'Throws · lifting · shooting · archery · judo — power and precision',
  '수영부':'Swim squad',
  '자유형·배영·평영·접영·계영 — 물':'Free · back · breast · fly · relay — water',
  '종합':'All-round',
  '종합 (특기 없음)':'All-round (no specialty)',
  '모든 갈래가 같습니다 — 예전과 같은 클럽':'Every group trains alike — the club as before',
  '아직 안 정했습니다 — 모든 갈래가 같습니다':'Not chosen yet — every group is the same',
  '아직 안 정했습니다 — 정하면 그 갈래가 빨리 큰다':'Not chosen — pick one and that group grows faster',
  '확인 정한다   취소 돌아가기':'Confirm to set   Cancel back',
  '없는 갈래입니다':'No such group',
  /* 스카우트(4L_scouts) */
  '스카우트 복귀 — 후보 %1명 발견':'scout is back — %1 prospect(s) found',
  '주급 합':'Total wages',
  '이(가) 간다':'will go',
  '스카우트 고용':'Hire scouts',
  '보는 눈':'Eye',
  ...
  '스카우트':'Scouts',
  '김 실장':'Mr. Kim',
  '국내를 오래 봤다':'Has watched the home scene for years',
  '탄 선생':'Mr. Tan',
  '아시아 학교를 다 안다':'Knows every school in Asia',
  '뒤발':'Duval',
  '세계 어디든 간다':'Will travel anywhere',
  '박 코치':'Coach Park',
  '어린 선수를 알아본다':'Spots the young ones',
  '노박':'Novak',
  '싸고 발이 넓다':'Cheap and well connected',
  '한 선배':'Elder Han',
  '눈이 아주 밝다':'An exceptionally sharp eye',
  '확인 고용/해고   취소 돌아가기':'Confirm hire/release   Cancel back',
  /* 이적 협상(34_market) */
  '제값을 낸다':'Pay the asking price',
  '조금 깎는다':'Haggle a little',
  '많이 깎는다':'Haggle hard',
  '성사':'Chance',
  '실패하면 놓친다':'fail and you lose them',
  '확실하다':'certain',
  '제안이 거절됐습니다 — 다른 클럽으로 갔습니다':'Offer rejected — they signed elsewhere',
  '영입 완료':'Signed',
  /* 계약(4K_contract) */
  '계약 만료로 떠남':'Left on a free transfer',
  '계약':'Contract',
  '요구':'Asks',
  '계약금':'Signing fee',
  '년':'y',
  '올해 끝나는 계약':'Expiring this year',
  '올해 끝나는 계약 없음':'No contracts expiring',
  '자금이 부족합니다':'Not enough funds',
  '선수가 재계약을 거절했습니다':'The athlete refused to re-sign',
  '재계약':'Re-signed',
  '모두 여유 있음':'All have time left',
  '올해 끝나는 계약 %1 — %2 외':'%1 expiring this year — %2 and others',
  '확인 재계약   취소 돌아가기':'Confirm re-sign   Cancel back',
  /* 이사회(4I_board) */
  '적자로 시즌을 마쳤습니다.':'You finished the season in debt.',
  '이사회가 승인하지 않습니다 (한도 %1)':'The board will not approve it (limit %1)',

  '신뢰':'Trust',
  '이사회 신뢰':'Board confidence',
  '이사회':'Board',
  '경질':'Sacked',
  '남는 것':'What you keep',
  '커리어 기록 · 명예의 전당 · 종족 도감':'Career records · Hall of Fame · Codex',
  '클럽만 바뀝니다':'Only the club changes',
  '새 클럽을 맡는다':'Take a new club',
  '든든하다':'Solid',
  '믿는다':'Trusting',
  '지켜본다':'Watching',
  '불안하다':'Uneasy',
  '한계다':'At the limit',
  '이사회가 만족합니다.':'The board is satisfied.',
  '이사회가 지켜보고 있습니다.':'The board is watching.',
  '이사회가 다시 믿기 시작했습니다.':'The board is starting to trust you again.',
  '이사회가 감독을 교체하기로 했습니다.':'The board has decided to replace you.',
  '마지막 경고입니다 (%1 / %2). 다음 시즌이 중요합니다.':'Final warning (%1 / %2). Next season matters.',
  '이사회가 불안해합니다 (경고 %1 / %2).':'The board is uneasy (warning %1 / %2).',
  /* 클럽 생활(4H_clublife) — 주간 사건 */
  '관계':'Rapport',
  '따른다':'Loyal',
  '서먹하다':'Distant',
  '등을 돌렸다':'Turned away',
  ...
  '팀 화합':'Team spirit',
  '결과':'Result',
  '▲▼ 고르기   확인 결정':'▲▼ Choose   Confirm to decide',
  '년차':'yr',
  '주':'wk',
  '끈끈하다':'Tight-knit',
  '삐걱인다':'Strained',
  ...
  'P%1 승리':'P%1 Wins', '%1위':'#%1', '준비되면':'When you are ready', '좌·우 아무 쪽이나 누르세요':'Press left or right to begin', '무승부':'Draw', '상금 +%1':'Prize +%1',
  ...
  '확인 한 번 더 — %1 방출': 'Press Confirm again to release %1',
  '확인 한 번 더 방출   취소 그만두기': 'Confirm again to release   Cancel to stop',
  '장착한다': 'Equip',
  '지금 착용 중인 것은 창고로 돌아갑니다': 'What is worn now goes back to storage',
  '합성한다': 'Fuse',
  '같은 등급 3개를 한 등급 위로': 'Three of the same grade become one grade up',
  '판다': 'Sell',
  '되돌릴 수 없습니다': 'Cannot be undone',
  '창고': 'Storage',
  '코인 %1': '%1 coins',
  '확인 한 번 더 — %1 판매': 'Press Confirm again to sell %1',
  '확인 한 번 더 판매   취소 그만두기': 'Confirm again to sell   Cancel to stop',
  '같은 장비 3개를 한 등급 위로': 'Three of the same item become one grade up',
  '같은 장비 3개가 필요합니다': 'You need three of the same item',
  '잠재치를 범위로 본다': 'See potential as a range',
  '전당에서 물려받기': 'Inherit from the Hall',
  '아직 없음 — 분야별로 3명까지': 'None yet — up to 3 per area',
  '오늘은 끝났습니다': 'Done for today',
  '아직 비어 있습니다': 'Still empty',
  ' · 받을 보상이 있습니다': ' · a reward is waiting',
  '시즌 마무리': 'Season wrap-up',
  ' (부상)': ' (injured)',
  ' (한계)': ' (at cap)',
  '직접 뛴다 · ': 'You run it · ',
  '출전 없음': 'Not entered',
  '어린 선수 위주 (잠재력 편차 큼)': 'Young players (wide potential spread)',
  '즉시 전력 위주': 'Ready-now players',
  '잠김': 'Locked',
  '  · 합성 가능': '  · can fuse',
  '전당의 선수에게서 자질을 받는다': 'Inherit a trait from a hall member',
  '확인 ': 'Confirm ',
  '확인 한 번 더 — %1 다시 뽑기': 'Press Confirm again to reroll %1',
  '타수 %1  ·  초당 %2': '%1 taps  ·  %2 / sec',
  '같은 쪽 %1': 'same side %1',
  ...
  'Lv.%1 부터': 'From Lv.%1',
  '%1 등급 종족만': '%1 grade species only',
  '%1 선수만': '%1 athletes only',
  '%1 먼저': '%1 first',
  '훈련 포인트 %1 필요': 'Needs %1 training points',
  '포인트 %1': '%1 points',
  '이미 배웠습니다': 'Already learned',
  '없는 스킬': 'No such skill',
  '아직 없음 — 임시 인력으로 나갑니다': 'None yet — temp staff will cover',
  '크게 자랄 수 있다': 'Room to grow a lot',
