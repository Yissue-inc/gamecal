...
class MeetResultScreen extends Screen0 {
  ...
  draw(u){
    ...
    /* ⛔ `meet.prize` 는 **계산해서 예산에 넣고는 화면에 안 쓰던 값**이었다(전 파일 검색).
       상금은 조용히 들어왔다 — 감독 모드에서 돈이 어디서 왔는지 모르면
       '대회를 잘 뛰면 살림이 편해진다'는 고리가 통째로 안 보인다. 승점 옆에 붙인다. */
    /* ⚠ 처음엔 승점 아래(y=58)에 놓았다가 **26px 짜리 승점 숫자에 그대로 깔렸다**
       (실측 스크린샷: '상금 +17' 이 '62' 를 가로질렀다). 큰 글자는 기준선 아래로 자란다 —
       자리를 옮길 땐 그 자리에 누가 사는지 재 보고 옮긴다. 라벨 줄 오른쪽 빈 자리로. */
    if(m.prize) txt(u, K('상금 +%1').replace('%1',
        (typeof MONEY==='function'? MONEY(m.prize) : String(m.prize))), VW-14, 32, 10, PAL.green, 'right', 700);
