...
const PAL = {
  ...
  /* `dim` 은 **어두운 판 위**에서만 읽힌다. 밝은/색 있는 바탕(선택된 행의 금빛 하이라이트,
     파란 탭 알약) 위에서는 대비가 1.5~2.4 로 떨어진다(실측 2026-09-04).
     그런 자리에는 이걸 쓴다 — 같은 '보조 정보' 인상을 유지하면서 읽히는 밝기다. */
  dimOn:'#cfd6e8',
  /* 같은 이유의 빨강 — 파란 알약 위에서 `red`(#ff5c5c) 는 2.26 이다 */
  redOn:'#ff9a9a',
};
