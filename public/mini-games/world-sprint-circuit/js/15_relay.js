...
const RELAY = {
  ...
  /* 인계 품질(q)이 다음 주자에게 주는 것 — [q=0 일 때, q=1 일 때]
     ⛔ 처음 값 carry [0.42, 1.02] · momentum [0.93, 1.09] 에서는 **인계를 잘하든 못하든 거의 같았다** —
        속도비로 넘긴 q≈0.9 와 구역 45% 에서 넘긴 q≈0.5 가 4x100 에서 0.4초(사다리 0.07) 차이.
        화면엔 속도 맞추기 게이지와 '지금 넘겨!' 가 있는데 그 기술이 기록에 거의 안 보였다.
     실측 2026-09-13 (depth_browser · 속도비 0.72 / 구역 45% / 구역 10% / 구역 85%):
       [0.42,1.02]·[0.93,1.09]  4x100 37.47/37.88/37.52/37.98 · 4x400 150.84/152.26/153.96/150.94
       [0.30,1.02]·[0.86,1.10]  4x100 37.46/38.02 · 4x400 150.81/153.40
       [0.25,1.02]·[0.80,1.10]  4x100 37.49/38.22 · 4x400 151.39/154.73
       **[0.20,1.03]·[0.75,1.11]**  4x100 37.46/38.34/37.58/38.50 · 4x400 151.35/155.25/160.01/151.12
     → 잘 넘긴 기록은 그대로, 엉성한 인계만 0.9초(4x100)·3.9초(4x400) 잃는다. 깊이 0.07 → 0.16~0.20 */
  carry: [0.20, 1.03],
  momentum: [0.75, 1.11],
};
...
class RelayEvent extends SprintEvent {
  ...
  handoff(tMs){
    ...
    nxt.speed = lerp(r.speed*RELAY.carry[0], r.speed*RELAY.carry[1], q);
    /* 좋은 인계는 '탄력'을 남긴다 — 몇 초 동안 목표 속도가 올라간다.
       나쁜 인계는 반대로 깎인다. 이게 계주가 개인기록 합보다 빠른/느린 이유다. */
    nxt.momentum = lerp(RELAY.momentum[0], RELAY.momentum[1], q); nxt._mom0 = nxt.momentum; nxt.momentumT = 0;
  ...
  drawUI(u){
    ...
    if(this.phase==='SET'){
      ...
      HUD.rhythm(u, { strides:(this.player&&this.player.combo)||0, nextSide:-r.lastSide||1, phaseErr:err, form:r.form, rate:(r.strideRate||0)});
