...
**P26 — 공룡·로봇·곤충 세계 배경 보강: 완료 ✓** → `prompts/P26_새세계_배경_보강.md`

`dino-sky`·`city-sky`·`bug-sky`를 명암 그라데이션과 저채도 구름/별/잎사귀 빛으로 교체했다
(60×34 축소 고유색 406/808/522). `city-crate`는 채워진 목재 상자, `city-factory`는 굴뚝·창문 격자·파이프
전경으로 교체했고 로봇도시 전경 레이어에 상자를 복구했다. PNG와 런타임 WebP를 함께 갱신하고 `stamp.py`로 개정값을 반영했다.
