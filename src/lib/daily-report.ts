...
export async function buildDailyReport(): Promise<DailyReport> {
  ...
  const [
    ...
    countTable(admin, 'event_entries'),
    countTable(admin, 'event_entries', 'entered_at', start),
    countTable(admin, 'event_entries', 'entered_at', previousStart, start),
  ...
  return {
    ...
    metrics: [
      ...
      { label: 'Launch giveaway entries total', value: eventEntriesTotal.value },
      { label: 'Launch giveaway entries new', value: eventEntriesToday.value, previous: eventEntriesPrevious.value },
